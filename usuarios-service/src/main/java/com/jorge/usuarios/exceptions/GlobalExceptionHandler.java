...
public class GlobalExceptionHandler {
    ...
    public GlobalExceptionHandler(JwtUtil jwtUtil) {
    ...
    private void registrarIncidencia(Exception ex) {
        ...
        // 1. Extraemos datos de la petición de forma segura e independiente
        String endpoint = extraerEndpoint(attrs);
        Long idUsuario = extraerIdUsuario(attrs); // Adiós a la asignación inútil

        // 2. Construimos el payload de la incidencia
        java.util.Map<String, Object> incidenciaJson = construirPayloadIncidencia(ex, endpoint, idUsuario);

        // 3. La enviamos al otro microservicio
        enviarIncidencia(incidenciaJson);
    }

    private String extraerEndpoint(ServletRequestAttributes attrs) {
        return (attrs != null) ? attrs.getRequest().getRequestURI() : "Desconocido";
    }

    private Long extraerIdUsuario(ServletRequestAttributes attrs) {
        if (attrs == null) return 0L;

        String authHeader = attrs.getRequest().getHeader("Authorization");
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            return 0L; // Retorno temprano, no hace falta variable temporal
        }

        try {
            String token = authHeader.substring(7);
            Long id = jwtUtil.obtenerIdUsuario(token);
    ...
    private java.util.Map<String, Object> construirPayloadIncidencia(Exception ex, String endpoint, Long idUsuario) {
        ...
        incidenciaJson.put("traza", obtenerTrazaRecortada(ex));
    ...
    private void enviarIncidencia(java.util.Map<String, Object> incidenciaJson) {
